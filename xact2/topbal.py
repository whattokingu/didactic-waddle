...
def topbalance():
	...
	top_bal_stmt = session.prepare(
		...
		SELECT c_w_id, c_d_id, c_id, c_balance, c_first, c_middle, c_last
		FROM customer
		WHERE c_w_id = ? AND c_d_id = ? AND c_balance > ?
	...
	for i in range(1,11):
		for j in range (1,11):
			query = session.execute(top_bal_stmt, [i, j, top_bal])
			if len(query) > 0
				res = list(query)
				cust_res.append(res)
				top_bal = res[0].c_balance
	
	result = []

	if len(cust_res) > 0
		sort_res = cust_res + current_top
		sort_res = list(sorted(sort_res, key = lambda c: -c.c_balance))
		result = sort_res
	else
		result = current_top
